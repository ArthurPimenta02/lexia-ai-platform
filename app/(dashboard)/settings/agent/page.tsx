...
export default async function SettingsAgentPage() {
      ...
      <div className="max-w-2xl">
        <div className="border-b border-border px-6 py-5">
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Configure o comportamento do agente de atendimento do seu escritorio.
          </p>
        </div>
        <div className="p-6">
  ...
  const initial = 'error' in result ? null : result.agent
  ...
  return (
    <div className="max-w-2xl">
      ...
      {error || !initial ? (
        <div className="p-6">
          <p className="rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
            {error ?? 'Nao foi possivel carregar as configuracoes do agente.'}
          </p>
        </div>
      ) : (
